...
const GestionDemandesFormateurs = () => {
    ...
    const fetchDemandes = async () => {
        try {
            ...
            const response = await axios.get(
                'http://localhost:5000/api/formateurs/en-attente',
                {
    ...
    const handleAction = async (id, statut) => {
        ...
        try {
            ...
            const payload = {
                statut,
                createAccount: statut === 'approuve' ? createAccount : false,
                password: statut === 'approuve' && createAccount ? password : undefined
            };

            await axios.patch(
                `http://localhost:5000/api/formateurs/${id}/statut`,
                payload,
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="mb-8">
                    <h1 className="text-4xl font-bold mb-2 bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                        Demandes de Formateurs Externes
                    </h1>
                    <p className="text-slate-600 dark:text-slate-400">
                        Gérez les candidatures des formateurs souhaitant rejoindre votre plateforme
                    </p>
                </div>
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                                    ...
                                    <div className="flex-1 space-y-3">
                                        ...
                                        <div className="flex items-start">
                                            <Tag className="w-4 h-4 mr-2 text-green-500 dark:text-green-400 mt-1 flex-shrink-0" />
                                            <div className="flex-1">
                                                <p className="text-sm text-slate-600 dark:text-slate-400 mb-1">Compétences :</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {demande.mots_cles.split(',').map((mot, index) => (
                                                        <span
                                                            key={index}
                                        ...
                                        {demande.remarques && (
                                            <div className="bg-slate-100 dark:bg-slate-700/50 rounded-lg p-3">
                                                <p className="text-sm text-slate-600 dark:text-slate-400 mb-1">Remarques :</p>
                                                <p className="text-sm text-slate-800 dark:text-slate-300">{demande.remarques}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex lg:flex-col gap-3">
                                        ...
                                        <button
                                            onClick={() => {
                                                setSelectedDemande(demande);
                                                handleAction(demande.id, 'approuve');
                                            }}
                                        ...
                                        <button
                                            onClick={() => {
                                                if (confirm('Êtes-vous sûr de vouloir rejeter cette demande ?')) {
                                                    handleAction(demande.id, 'rejete');
                                                }
                                            }}
                                            disabled={actionLoading}
                    ...
                    <div className="bg-gray-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white font-medium">{formatDate(selectedDemande.created_at)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {selectedDemande.mots_cles.split(',').map((mot, index) => (
                                        <span
                                            key={index}
                            ...
                            {selectedDemande.remarques && (
                                <div>
                                    <h3 className="text-lg font-semibold text-white mb-3">Remarques / Présentation</h3>
                                    <div className="bg-gray-700/50 rounded-lg p-4">
                                        <p className="text-gray-300">{selectedDemande.remarques}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="border-t border-gray-700 pt-6">
                                ...
                                <div className="bg-gray-700/50 rounded-lg p-4 mb-4">
                                    ...
                                    <p className="text-sm text-gray-400 mt-2 ml-8">
                                        Permet au formateur de se connecter à la plateforme
                                    </p>
                                </div>
                                ...
                                {createAccount && (
                                    <div className="bg-gray-700/50 rounded-lg p-4">
                                        ...
                                        <p className="text-xs text-gray-400 mt-2">
                                            Si vide, le mot de passe par défaut sera : formateur123
                                        </p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex gap-4 pt-4">
                                <button
                                    onClick={() => handleAction(selectedDemande.id, 'approuve')}
                                    disabled={actionLoading}
                                ...
                                <button
                                    onClick={() => {
                                        if (confirm('Êtes-vous sûr de vouloir rejeter cette demande ?')) {
                                            handleAction(selectedDemande.id, 'rejete');
                                        }
                                    }}
                                    disabled={actionLoading}
...
export default GestionDemandesFormateurs;
